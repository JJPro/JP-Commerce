...
adminpage||"undefined"===typeof wp||!wp.heartbeat||wp.heartbeat.connectNow();b.fadeOut(200,function(){b.addClass("hidden").css("display","");a("#wp-auth-check-frame").remove();a("body").removeClass("modal-open")})}function g(){var a=parseInt(window.authcheckL10n.interval,10)||180;l=(new Date).getTime()+1E3*a}var b,l;a(document).on("heartbeat-tick.wp-auth-check",function(a,c){"wp-auth-check"in c&&(g(),!c["wp-auth-check"]&&b.hasClass("hidden")?m():c["wp-auth-check"]&&!b.hasClass("hidden")&&k())}).on("heartbeat-send.wp-auth-check",
function(a,b){(new Date).getTime()>l&&(b["wp-auth-check"]=!0)}).ready(function(){g();b=a("#wp-auth-check-wrap");b.find(".wp-auth-check-close").on("click",function(){k()})})})(jQuery);

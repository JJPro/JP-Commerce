...
clickable:".dz-clickable",dictInvalidFileType:"Invalid file type.",dictResponseError:"Upload Failed. \nError Code: {{statusCode}}.",dictMaxFilesExceeded:"Exceeded number of files allowed."});var b=Dropzone.instances[0];b.on("addedfile",function(c){a("#media-upload-wrap").append(a("#upload-indicator-wrap"));if(this.files.length){var b,m;b=0;for(m=this.files.length;b<m-1;b++)this.files[b].name===c.name&&(this.removeFile(c),alert("Another file with the same name already exists!"))}});b.on("maxfilesexceeded",
function(c){this.removeFile(c)});b.on("removedfile",function(c){4==this.files.length&&this.enable();console.log(c);a.ajax({url:jc_data.ajaxurl,method:"post",data:{action:"jp_commerce_remove_artwork_media",nonce:e,name:c.name,post_id:f,author_id:g}}).done(function(){alert("file is successfully deleted.")}).fail(function(){alert("failed to delete file from server.")})});b.on("sending",function(c,b,a){a.append("action","jp_commerce_upload_artwork_media");a.append("post_id",f);a.append("author_id",g);
a.append("nonce",e)});b.on("success",function(a,b){console.log(b)});var h=existing_thumbnails.length;console.log("file count: "+h);b.options.maxFiles-=h;for(var k,l=0;l<h;l++){k=existing_thumbnails[l];var d={name:k.name,size:123};console.log("mockFile: "+d.size);b.files.push(d);b.emit("addedfile",d);b.emit("thumbnail",d,k.url);b.emit("complete",d)}});

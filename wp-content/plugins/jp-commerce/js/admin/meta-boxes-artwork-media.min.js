...
c.on("maxfilesexceeded",function(a){this.removeFile(a)});c.on("removedfile",function(a){console.log(this.files.length);4==this.files.length&&this.enable();console.log(a);b.ajax({url:jc_data.ajaxurl,method:"post",data:{action:"jp_commerce_remove_artwork_media",nonce:f,name:a.name,post_id:g,author_id:h}}).fail(function(){alert("failed to delete file from server.")})});c.on("sending",function(a,b,c){c.append("action","jp_commerce_upload_artwork_media");c.append("post_id",g);c.append("author_id",h);c.append("nonce",
f)});c.on("success",function(a,b){b.success||alert(b.data)});var k=existing_thumbnails.length;console.log("file count: "+k);c.options.maxFiles-=k;for(var l,m=0;m<k;m++){l=existing_thumbnails[m];var e={name:l.name,size:123};console.log("mockFile: "+e.size);c.files.push(e);c.emit("addedfile",e);c.emit("thumbnail",e,l.url);c.emit("complete",e)}});
